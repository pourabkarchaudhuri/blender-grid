...
def enqueue():
    ...
    job = redis_queue.enqueue(some_long_function, data["sas_url"], job_timeout=1000000)
    return jsonify({"job_id": job.id,"n_jobs":len(redis_queue)})
...
@app.route("/status")
def status():
    return jsonify({"n_jobs":len(redis_queue)})
